...
from turbo_funcs import T2VTurboVC2Pipeline
# import ipdb
# st = ipdb.set_trace
...
def run_training(args, **kwargs):
    ...
    # replace model.model.diffusion_model.unet with the loaded unet
            ...
            with accelerator.accumulate(peft_model):    # gradient accumulation
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % checkpointing_steps ==0:
                        ...
                        #    model_path = os.path.join(output_dir, f"model_{global_step}.pt")
    ...
    if accelerator.is_local_main_process:
        unwrapped_model = accelerator.unwrap_model(peft_model)
        # peft_state_dict = peft.get_peft_model_state_dict(unwrapped_model)
        model_path = os.path.join(output_dir, f"model_final.pt")
        torch.save(unwrapped_model.state_dict(), model_path)
    logger.info("Saving checkpoing end")
