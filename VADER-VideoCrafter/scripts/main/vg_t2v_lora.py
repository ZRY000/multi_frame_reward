...
from funcs import batch_ddim_sampling, vg_batch_ddim_sampling, get_critic_optimizer, CriticDataset, compute_actor_loss, compute_critic_loss, compute_target_values
from utils.utils import instantiate_from_config
from lvdm.models.critic import Critic, MultiFrameCritic
from lvdm.models.reward import vllava_rew_fn
...
def run_training(args, **kwargs):
    ...
    if args.use_wandb:
        if accelerator.is_main_process:
            ...
            exp_name = f"vc_vg_{args.backprop_mode}"
    ...
    peft_model, critic, target_critic, optimizer, critic_optimizer, rew_fn = accelerator.prepare(
        peft_model, critic, target_critic, optimizer, critic_optimizer, rew_fn)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        # for idx, step in enumerate(range(default_iterations)): # default number of iterations is 100
            ...
            with accelerator.accumulate(peft_model, critic):    # gradient accumulation
                with accelerator.autocast():            # mixed precision
                    ...
                    for i in range(len(backprop_idxs)-1):
                        ...
                        if is_last:
                            if args.reward_fn == "aesthetic" or args.reward_fn == "objectDetection" or args.reward_fn == "compression_score":
                                ...
                                reward = rew_fn(z_decode, batch_prompts)
                        ...
                        step_loss_list.append(actor_loss.clone().detach())
                    ...
                    rank_loss = sum(step_loss_list)/len(step_loss_list)
                    step_loss = accelerator.gather(rank_loss.repeat(args.train_batch_size)).mean()
                    step_loss_list = []
                    ...
                    for i in range(critic_iterations):
                        ...
                        for j in range(len(critic_dataset)):
                            # critic_loss = critic.loss(traj=critic_dataset.traj_ds[j], 
                            #                             target_values=critic_dataset.target_values_ds[j],
                            #                             prompts=pure_prompts)
            ...
            if accelerator.sync_gradients:
                global_step += 1
                log_dict_step = {"step_loss": step_loss.item(),
                                    "rank0_loss": rank_loss.item(),
                ...
                if global_step % checkpointing_steps ==0:   
                    ...
                    if accelerator.is_local_main_process and accelerator.sync_gradients:
                        ...
                        peft_model_dir = os.path.join(output_dir, "diffusion_model")
                        os.makedirs(peft_model_dir, exist_ok=True)
                        peft_model_path = os.path.join(peft_model_dir, f"peft_model_{global_step}.pt")
                        ...
                        critic_model_path = os.path.join(critic_model_dir, f"critic_{global_step}.pt")
            ...
            if should_sample(global_step, validation_steps, args.is_sample_preview):
                ...
                if accelerator.is_local_main_process:
                    with torch.no_grad():
                        ...
                        if args.decode_frame in ["fml", "all", "alt"]:
                            ...
                            # save as image
                            train_gen_frames = Image.fromarray((((val_img + 1.0)/2.0)[0].permute(1,2,0).detach().cpu().clamp(0, 1).numpy() * 255).astype(np.uint8))
                ...
                # release the memory
                del z, z_decode, traj_decode, val_img
