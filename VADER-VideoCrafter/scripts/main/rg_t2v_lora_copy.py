...
def run_training(args, **kwargs):
    ...
    # ==== load image tower ====
    if rew_model.model.config.mm_image_tower is not None:
        image_tower = rew_model.model.get_image_tower()
    ...
    # rew_model, _, _, _ = deepspeed.initialize(
    #     model=rew_model,
    #     config=ds_config,
    #     model_parameters=None
    # )
    # rew_model_engine = rew_model
    ...
    # internv2, _, _, _ = deepspeed.initialize(
    #     model=internv2,
    #     config=ds_config,
    #     model_parameters=None
    # )
    # clip, _, _, _ = deepspeed.initialize(
    #     model=clip,
    #     config=ds_config,
    #     model_parameters=None
    # )
    # hps, _, _, _ = deepspeed.initialize(
    #     model=hps,
    #     config=ds_config,
    #     model_parameters=None
    # )
            ...
            with accelerator.accumulate(peft_model):    # gradient accumulation
                with accelerator.autocast():            # mixed precision
                    ...
                    if args.reward_fn == 'vllava':
                        ...
                        reward = (F.sigmoid(vllava_reward) + F.sigmoid(frame_reward)) / 2.0
                        # reward = vllava_reward

                        loss = - ((1 - unsafe_mask) * reward + unsafe_mask * cost).mean() 
                ...
                step_loss = accelerator.gather(loss.clone().repeat(args.train_batch_size)).mean().detach()
