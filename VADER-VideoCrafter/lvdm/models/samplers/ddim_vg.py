...
class VGDDIMSampler(object):
    ...
    def rollout(self,
        ...
        samples, intermediates = self.ddim_rollout(conditioning, size,     # samples: batch, c, t, h, w
                                                    callback=callback,
                                                    img_callback=img_callback,
                                                    quantize_denoised=quantize_x0,
                                                    mask=mask, x0=x0,
    ...
    def ddim_rollout(self, cond, shape,
                    z=None, ddim_use_original_steps=False,
                    callback=None, timesteps=None, quantize_denoised=False,
                    mask=None, x0=None, img_callback=None, log_every_t=100,
                    temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                    unconditional_guidance_scale=1., unconditional_conditioning=None, verbose=True,
                    cond_tau=1., target_size=None, start_timesteps=None, start_idx=None, end_idx=None,
                    **kwargs):
        device = self.model.betas.device        
        # print('ddim device', device)
        b = shape[0]
        if z is None:   # z is the input to the rollout
            img = torch.randn(shape, device=device)
        else:
            img = z
        
        if timesteps is None:
            timesteps = self.ddpm_num_timesteps if ddim_use_original_steps else self.ddim_timesteps     # ddim_use_original_steps = False, timesteps = None -> timesteps = self.ddim_timesteps
        elif timesteps is not None and not ddim_use_original_steps:
            subset_end = int(min(timesteps / self.ddim_timesteps.shape[0], 1) * self.ddim_timesteps.shape[0]) - 1
            timesteps = self.ddim_timesteps[:subset_end]
        ...
        if verbose:
            # iterator = tqdm(time_range, desc='DDIM Sampler', total=total_steps)
            iterator = tqdm(time_range[start_idx:end_idx], desc='DDIM Sampler')
        else:
            iterator = time_range[start_idx:end_idx]
        assert len(iterator) == end_idx - start_idx
        ...
        if self.training_mode == True:
            if self.backprop_mode == 'last':
                backprop_cutoff_idx = len(iterator) - 1
            elif self.backprop_mode == 'rand':
                backprop_cutoff_idx = random.randint(0, len(iterator) - 1)
            elif self.backprop_mode == 'specific':
                backprop_cutoff_idx = 15

        for i, step in enumerate(iterator):
            index = total_steps - i - start_idx - 1
            ts = torch.full((b,), step, device=device, dtype=torch.long)
