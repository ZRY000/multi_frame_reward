...
class RGDDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      cond_tau=1., target_size=None, start_timesteps=None, neg_cond=None,
        ...
        if self.training_mode == True:
            early_stop = self.ddim_num_steps
            ...
            elif self.backprop_mode == "refl":
                early_stop = random.randint(self.ddim_num_steps//3,(self.ddim_num_steps//3)*2)
                backprop_cutoff_idx = early_stop - 1 
        ...
        for i, step in enumerate(iterator):
            ...
            if self.training_mode == True:
                if i >= backprop_cutoff_idx and i <= early_stop :
            ...
            with forward_context():
                outs = self.p_sample_ddim(img, cond, ts, index=index, use_original_steps=ddim_use_original_steps,
                                        ...
                                        unconditional_guidance_scale=unconditional_guidance_scale,
                                        unconditional_conditioning=unconditional_conditioning,
                                        x0=x0, negtive_condition=neg_cond, safety_momentum=safety_momentum,
                                        **kwargs)
            
            img, pred_x0, safety_momentum = outs
            ...
            # add refl directly predict original
            if self.backprop_mode == "refl" and i == early_stop :
                img = pred_x0
                break
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      ...
                      uc_type=None, conditional_guidance_scale_temporal=None, negtive_condition=None,
                      safety_momentum=None, **kwargs):
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            # text cfg
            if negtive_condition is not None:
                if safety_momentum is None:
                    safety_momentum = torch.zeros_like(e_t)
                e_t_neg_cond= self.model.apply_model(x, t, negtive_condition, **kwargs).detach() # TODO: check whether detach is necessary
                #equation 6
                scaler1=torch.clamp(torch.abs(e_t-e_t_neg_cond)*hypers['sld_guidance_scale'],max=1.)
                #equation 6
                scaler2=torch.where(e_t-e_t_neg_cond>=hypers['sld_threshold'], torch.zeros_like(e_t), scaler1)
                
                e_t_neg_guidance=torch.mul(e_t_neg_cond-e_t_uncond, scaler2)
                e_t_neg_guidance = e_t_neg_guidance + safety_momentum*hypers['sld_momentum_scale']
                safety_momentum = hypers['sld_mom_beta']*safety_momentum + e_t_neg_guidance*(1-hypers['sld_mom_beta'])
            
            if negtive_condition is not None:
                #print("MANUS!!")
                #print(e_t_neg_guidance)
                #input()
                #skip warmups
                #print(e_t)
                e_t = e_t - e_t_neg_guidance*1.15
                #print("NEW")
                #print(e_t)
                #input()
        ...
        return x_prev, pred_x0, safety_momentum
