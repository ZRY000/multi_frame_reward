...
from opensora.utils.train_utils import get_critic_optimizer
import peft
import torchvision
...
from opensora.models.critic.critic import Critic
...
def pick_score_loss_fn(inference_dtype=None, device=None):
    ...
    processor = AutoProcessor.from_pretrained("/home/juntao/Models/CLIP-ViT-H-14-laion2B-s32B-b79K", torch_dtype=inference_dtype)
    print("CLIP good \nCLIP good \nCLIP good \nCLIP good \nCLIP good \nCLIP good \nCLIP good \nCLIP good \nCLIP good \nCLIP good \n")
    model = AutoModel.from_pretrained("/home/juntao/Models/PickScore_v1", torch_dtype=inference_dtype).eval().to(device)
...
def main():
    ...
    # text_encoder = AutoTokenizer.from_pretrained("/home/juntao/Models/DeepFloyd/t5-v1_1-xxl", device=device)
    # text_encoder =
    from opensora.models.vae.vae import VideoAutoencoderPipeline
    # vae = VideoAutoencoderPipeline.from_pretrained("/home/juntao/Models/OpenSora-VAE-v1.2").to(device, dtype).eval()
    ...
    if image_size is None:
        # resolution = cfg.get("resolution", None) #哪里改了
        # aspect_ratio = cfg.get("aspect_ratio", None) #这里改了
        resolution = "240p"
        aspect_ratio = "3:4"
        assert (
    ...
    # num_frames = get_num_frames(cfg.num_frames) 这里改了？哪里改了？cqx改的.
    num_frames = 51
    ...
    print("critic successfully defined")
    ...
    if not cfg.get("is_vader_training", False): # if it is inference only mode
        ...
        #这里改了
        if prompts is None:
            prompts = load_prompts("../assets/chatgpt_custom_human_fashion.txt",start_idx,None)
        # if prompts is None:
    ...
    # critic_optimizer
    # optimizer takes in the parameters of critic
    critic_optimizer = get_critic_optimizer(cfg, critic)
    ...
    for epoch in range(first_epoch, cfg.get("num_train_epochs", 200)):
        ...
        for idx, step in enumerate(range(default_iterations)):
            ...
            # == get json from prompts ==
            # refs and ms are obtained from batch_prompts, because the default is null string
            # if config and prompts don't contain information about refs and ms, then both are empty strings
            ...
            with accelerator.accumulate(peft_model):    # gradient accumulation
                with accelerator.autocast():            # mixed precision                    
                    # == sampling ==
                    horizon = 5 # to be modified by cfg.get()
                    sample_size = 10 # # to be modified by cfg.get()
                    
                    z_samples = [torch.randn(len(batch_prompts), vae.out_channels, *latent_size, device=device, dtype=dtype) for _ in range(sample_size)]
                    # z = torch.randn(len(batch_prompts), vae.out_channels, *latent_size, device=device, dtype=dtype)
                    masks = apply_mask_strategy(z, refs, ms, loop, align=align) # loop = 1 for vader training


                    windows = scheduler.get_windows(z, horizon, device, additional_args=model_args)
                    
                    for w_idx, window in enumerate(windows):
                        new_samples = []
                        traj_samples = []
                        for z in z_samples:
                            z, traj = scheduler.sample(
                                peft_model.module,
                                text_encoder,
                                z=z,
                                prompts=batch_prompts_loop,
                            ...
                            new_samples.append(z)
                            traj_samples.append(traj)
                        z_samples = new_samples
                        ...
                        # TODO: update the critic dataset

                        # TODO: update the critic and target_critic
