...
def get_critic_optimizer(cfg, model):
    func_name = cfg.get("reward_fn", "aesthetic")
    
    if func_name == "aesthetic": 
        pass
    elif func_name == "hps":
        ...
        critic_optimizer = torch.optim.AdamW(
            [
                ...
                {"params": rest_params, "weight_decay": cfg.get("wd", None)},
            ],
            lr=cfg.get("critic_lr", None),
            betas=(cfg.get("beta1", None), cfg.get("beta2", None)),
            eps=cfg.get("eps", None),
        )
        return critic_optimizer
    elif func_name == "aesthetic_hps":
        pass
    elif func_name == "pick_score":
        pass
    else:
        raise NotImplementedError(f"Critic {func_name} is not implemented for Open-Sora.")
